...
// Context to store value→label mappings so SelectValue can show labels even
// after the popup unmounts (which loses SelectItemText registrations).
const SelectLabelCtx = React.createContext<React.MutableRefObject<Map<string, string>> | null>(null)
...
function Select<Value = string>(props: SelectPrimitive.Root.Props<Value>) {
  ...
  return (
    <SelectLabelCtx.Provider value={labelsRef}>
...
function SelectValue({ className, children, placeholder, ...props }: SelectPrimitive.Value.Props & { placeholder?: string }) {
  const labelsRef = React.useContext(SelectLabelCtx)
  // Render via render-prop to access the current value and show the label
  return (
    <SelectPrimitive.Value
      ...
      {...props}
    >
      {/* Base-UI passes the selected value string as children here */}
      {(value: string | undefined) => {
        if (!value) return <span className="text-muted-foreground">{placeholder ?? ""}</span>
        const label = labelsRef?.current.get(value) ?? value
...
function SelectItem({
  ...
  // Register this item's label so SelectValue can look it up after popup unmounts
  React.useEffect(() => {
    if (labelsRef && value !== undefined) {
